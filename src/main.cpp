...
{ //*** system switched to active low. so RISE and FALL values are inverted.
  sensorTriggerHistory[sensorTriggerHistory_pointer] = edge;

  if (sensorTiggerHistory_pattern == PATTERN_EMPTY)
  { // wich means sensorTriggerHistory_pointer = 0
    if (edge == LEFT_FALL)
    { // L detected
      logln("L detected");
      sensorTiggerHistory_pattern = PATTERN_CARGO_MOVING_FORWARD;
    }
    else if (edge == RIGHT_FALL)
    { // R detected
      logln("R detected");
  ...
  if (sensorTiggerHistory_pattern == PATTERN_CARGO_MOVING_FORWARD)
  { // wich means sensorTriggerHistory_pointer = 1,2, or 3
    if (sensorTriggerHistory_pointer == 1 && edge == RIGHT_FALL)
    { // LR detected
      logln("LR detected");
    }
    else if (sensorTriggerHistory_pointer == 2 && edge == LEFT_RISE)
    { // LRl detected
      logln("LRl detected");
    }
    else if (sensorTriggerHistory_pointer == 3 && edge == RIGHT_RISE)
    { // LRlr detected
      logln("LRlr detected");
  ...
  if (sensorTiggerHistory_pattern == PATTERN_CARGO_MOVING_BACKWARD)
  { // wich means sensorTriggerHistory_pointer = 1,2, or 3
    if (sensorTriggerHistory_pointer == 1 && edge == LEFT_FALL)
    { // RL detected
      logln("RL detected");
    }
    else if (sensorTriggerHistory_pointer == 2 && edge == RIGHT_RISE)
    { // RLr detected
      logln("RLr detected");
    }
    else if (sensorTriggerHistory_pointer == 3 && edge == LEFT_RISE)
    { // RLrl detected
      logln("RLrl detected");
...
void loop()
{
  ...
  if (sensor_left_currentState != sensor_left_previousState)
  { // left sensor detected a change
    isEdgeDetected = true;
    sensor_left_previousState = sensor_left_currentState;

    if (sensor_left_currentState == HIGH)
    { // rising edge detected from the left sensor
      updateSensorTriggerHistory(LEFT_RISE);
    }
    else
    { // sensor_left_currentState == LOW _ falling edge detected from the left sensor
      updateSensorTriggerHistory(LEFT_FALL);
    }
  }

  if (sensor_right_currentState != sensor_right_previousState)
  { // right sensor detected a change
    isEdgeDetected = true;
    sensor_right_previousState = sensor_right_currentState;

    if (sensor_right_currentState == HIGH)
    { // rising edge detected from the right sensor
      updateSensorTriggerHistory(RIGHT_RISE);
    }
    else
    { // sensor_right_currentState == LOW _ falling edge detected from the right sensor
      updateSensorTriggerHistory(RIGHT_FALL);
    }
  }

  if (isEdgeDetected)
  { // we check patterns only when the system detects an edge
    ...
    case PATTERN_CARGO_MOVED_FORWARD:
      if (previousWeight != 0)
      { // if previousWeight==0 it means it's either the first weight or we had a cargo moved backward before this.
        if (isWeightQueueFull)
        { // if the flag is on, send the oldest weight to pc because it will be overwritten.
          log("***this weight sent to the pc:  ");
          logln(weightQueue[weightQueue_pointer]);
          Serial3.println(weightQueue[weightQueue_pointer]); //*majid*  this will send the result to final pc
        }
        weightQueue[weightQueue_pointer] = previousWeight;
        if (weightQueue_pointer == weightQueue_size - 1)
        { // the queue is full. move pointer to 0 and turn the flag on.
  ...
  {
    //log("rached new line");
    //majid Serial.println("bascule serial data input: " + basculeSerialDataInput);
    basculeSerialDataInput = basculeSerialDataInput.substring(4); //majid basculeSerialDataInput.indexOf("p+  " + 4));
    lastBasculeNumber = basculeSerialDataInput.toInt();
    //majid Serial.print("bascule serial number: ");
    //majid Serial.println(lastBasculeNumber);
...
void serialEvent2()
{ // serial data received from the bascule
